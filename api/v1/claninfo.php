<?php
	...
	// выводим ћануал, если нет параметров
	if (!isset($_GET['clanName']) and !isset($_GET['clanTag'])) {
		clan_manual_go();
		exit();
	}
	...
	while($data = mysql_fetch_array($qr_result)){ 
		...
		echo('<td>' . $data['effRating'] / $data['number'] . '</td>');
		echo('<td>' . $data['karma'] / $data['number'] . '</td>');
		echo('<td>' . $data['prestigeBonus'] / $data['number'] . '</td>');
		echo('<td>' . $data['gamePlayed'] / $data['number'] . '</td>');
		echo('<td>' . $data['gameWin'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalAssists'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalBattleTime'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalDeath'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalDmgDone'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalHealingDone'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalKill'] / $data['number'] . '</td>');
		echo('<td>' . $data['totalVpDmgDone'] / $data['number'] . '</td>');
		echo('</tr>');
	}
